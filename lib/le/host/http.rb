...
#!      attr_accessor :token, :queue, :started, :thread, :conn, :local, :debug, :ssl, :datahub_enabled, :dathub_ip, :datahub_port, :host_id, :custom_host, :host_name_enabled, :host_name
      ...
      def initialize(token, local, debug, ssl, datahub_endpoint, host_id, custom_host, udp_port, shift_age, shift_size)
          if local
          ...
          @logger_console = Logger.new(device, shift_age, shift_size)
          end
